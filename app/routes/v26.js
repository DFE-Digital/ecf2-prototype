...
module.exports = router => {
    ...
    // admin teacher replace deactivated TRN (merge if existing record, otherwise change TRN)
    // Prototype: TRNs in this list have an existing teacher record and trigger the merge path
    const EXISTING_ACTIVE_TRNS = ['1234567', '2345678']

    router.get(v + admin + 'teacher/enter-new-trn', (req, res) => {
        res.render(vGet + admin + 'teacher/enter-new-trn')
    })

    router.post(v + admin + 'teacher/enter-new-trn', (req, res) => {
        const trn = (req.body['replacement-trn'] || '').trim()
        const errors = {}
        const errorList = []

        if (!trn || !/^\d{7}$/.test(trn)) {
            errors.replacementTrn = 'Enter a valid TRN'
            errorList.push({
                text: errors.replacementTrn,
                href: '#replacement-trn'
            })
        }

        if (errorList.length) {
            return res.render(vGet + admin + 'teacher/enter-new-trn', {
                errors,
                errorList
            })
        }
        ...
        if (trn === '0000000') {
            return res.redirect(v + admin + 'teacher/cannot-use-trn')
        }

        req.session.data['replacement-trn'] = trn
        req.session.data['confirm-merge'] = null
        req.session.data['confirm-change-trn'] = null

        if (EXISTING_ACTIVE_TRNS.includes(trn)) {
            req.session.data['trn-replace-action'] = 'merge'
            return res.redirect(v + admin + 'teacher/merge-preview')
        }

        req.session.data['trn-replace-action'] = 'change'
        res.redirect(v + admin + 'teacher/confirm-change-trn')
    })

    router.get(v + admin + 'teacher/cannot-use-trn', (req, res) => {
        res.render(vGet + admin + 'teacher/cannot-use-trn')
    })

    router.get(v + admin + 'teacher/merge-preview', (req, res) => {
        if (!req.session.data['replacement-trn'] || req.session.data['trn-replace-action'] !== 'merge') {
            return res.redirect(v + admin + 'teacher/enter-new-trn')
        }
    ...
    router.get(v + admin + 'teacher/confirm-change-trn', (req, res) => {
        if (!req.session.data['replacement-trn'] || req.session.data['trn-replace-action'] !== 'change') {
            return res.redirect(v + admin + 'teacher/enter-new-trn')
        }

        res.render(vGet + admin + 'teacher/confirm-change-trn')
    })

    router.post(v + admin + 'teacher/confirm-change-trn', (req, res) => {
        const confirmChangeTrn = req.body['confirm-change-trn']
        const hasConfirmation = Array.isArray(confirmChangeTrn)
            ? confirmChangeTrn.includes('yes')
            : confirmChangeTrn === 'yes'
        ...
        if (!hasConfirmation) {
            return res.render(vGet + admin + 'teacher/confirm-change-trn', {
                errors: {
                    confirmChangeTrn: 'Confirm you want to change this teacher’s TRN'
                }
            })
        }

        res.redirect(v + admin + 'teacher/trn-changed')
    })

    router.get(v + admin + 'teacher/trn-changed', (req, res) => {
        res.render(vGet + admin + 'teacher/trn-changed')
    })
    ...
    router.post(v + admin + 'teacher/change-contract-period', (req, res) => {
        ...
        res.redirect(v + admin + 'teacher/change-contract-period-partnership')
    })
    ...
    // **********************
        ...
        router.get(v + admin + 'finance/void-declaration', (req, res) => {
            res.render(vGet + '/admin/finance/void-declaration', {
                query: req.query
            });
        })
